...
class SSDHead(nn.Layer):
    ...
    def __init__(self,
        ...
        for i, num_prior in enumerate(self.num_priors):
            ...
            self.box_convs.append(box_conv)  #定义一个列表用来储存box卷积的名字和卷积类型
            ...
            if not use_sepconv:
                ...
                score_conv = self.add_sublayer(
                    ...
                    SepConvLayer(
                        ...
                        padding=padding,
                        conv_decay=conv_decay))
            self.score_convs.append(score_conv)  # 定义一个列表来储存socreJ卷积的名字和卷积类型
    ...
    def forward(self, feats, image, gt_bbox=None, gt_class=None):
        ...
        cls_scores = []
        # i==0
        box_pred = self.box_convs[0](feats[0])  #box_convs列表里的第一个取出来 
        box_pred = self.box_quantize0(box_pred) # 添加量化结点 应该是根据名字 选择特定的卷积 添加量化节点
